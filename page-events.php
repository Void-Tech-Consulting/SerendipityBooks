...
<?php
//How to Get Order Detials from WooCommerce Checkout

//get order ID
$order_id = intval( str_replace( 'checkout/order-received/', '', $wp->request ) );

//get instance of order object and go through each item in order
// $order = wc_get_order( $order_id ); //other way to get order
$order = new WC_Order( $order_id );
$items = $order->get_items();

$productsInOrderSKUs = array();
foreach ( $items as $key => $item ) {
    // $product_id = $item['product_id']; //if doing by product id, but we used sku
    $product = $item->get_product();
    if ( $product ) {
        //array of products SKUS in order
        $productsInOrderSKUs[] = $product->get_sku();
    }
}
?>
...
<?php
//different way to do same thing as above
// Helpful Links: https://www.businessbloomer.com/woocommerce-easily-get-order-info-total-items-etc-from-order-object/
...
foreach ($order->get_items() as $key => $item) {
    $itemSKU = $item->get_product()->get_sku();
    $itemQuantity = get_quantity();
}
